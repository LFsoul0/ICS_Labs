...
#	##########################################
#	
#	change jmp predict logic: if the 2nd instruct before jxx is rmmovq, then predict pc according to valA of the rmmovq
#	(if rmmovq_valA > 0, predPC = jxx_valP, else predPC = jxx_valC)
#
...
word f_pc = [
	...
	M_icode == IJXX && M_Cnd && M_valE == 1 : M_valA;
...
word f_predPC = [
	f_icode == IJXX && E_icode == IRMMOVQ && E_valA > 0 : f_valP;
...
word d_valB = [
	D_icode == IJXX && M_icode == IRMMOVQ && M_valA > 0 : 1;
	D_icode == IJXX : 0;
