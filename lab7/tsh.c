...
void eval(char *cmdline) 
{
    ...
    /* parse and check if background process */
    ...
    /* else fork and execute the program */
	...
	/* wait foreground process terminate */
	waitfg(pid);
    }
...
void waitfg(pid_t pid)
{
    ...
    /* wait until pid no longer foreground */
...
void sigchld_handler(int sig) 
{
	...
	/* if stopped */
	    ...
	    if (job == NULL)
		app_error("get job from pid failed");
	    job->state = ST;
	}
